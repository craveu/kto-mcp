...
describe('PhotoAwardService', () => {
  ...
  describe('phokoAwrdList', () => {
    it('KtoHttpClient.request를 올바른 operation으로 호출해야 한다', async () => {
      ...
      const result = await service.phokoAwrdList({ numOfRows: 1 });
    ...
    it('빈 DTO로 호출 가능해야 한다 (모든 파라미터 선택)', async () => {
      ...
      await expect(service.phokoAwrdList({})).resolves.toBeDefined();
    });
  });
  ...
  describe('phokoAwrdSyncList', () => {
    it('KtoHttpClient.request를 올바른 operation으로 호출해야 한다', async () => {
      ...
      const result = await service.phokoAwrdSyncList({ showflag: '1' });
    ...
    it('syncModTime 파라미터가 KTO에 그대로 전달되어야 한다', async () => {
      ...
      await service.phokoAwrdSyncList({ syncModTime: '20250101000000' });
