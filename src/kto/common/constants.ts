...
// 같은 맵에 7가지 패턴이 공존한다: (1) V2 다국어 다중 path(KorService2, EngService2, ...), (2) V2 단독 사이드 서비스(KorWithService2), (3) V/숫자 suffix 없는 평면 형태(PhotoGalleryService1, GoCamping), (4) 단일 path + langCode 파라미터(Odii), (5) 단일 path + langDivCd 파라미터 다국어(MdclTursmService) — KTO가 임의 값 수용, 응답 lang은 server-normalized, (6) langDivCd fluid 다국어(WellnessTursmService), (7) 응답 필드 ko*/en* prefix 동시 노출 — 단일 호출로 양 언어 취득(PhokoAwrdService). KTO 통합 시리즈 SPEC-KTO-001~010 완료, 7-패턴 분류 체계 완성.
// 패턴별 가이드: 다국어 확장 → EngService2 패턴, 사이드 서비스 → KorWithService2 패턴, suffix 없는 신규 → GoCamping 패턴, langCode 파라미터 다국어 → Odii 패턴, langDivCd 파라미터 다국어 → MdclTursmService/WellnessTursmService 패턴 (6번 패턴), 응답 필드 prefix 다국어 → PhokoAwrdService 패턴 (7번 패턴).
// @MX:SPEC: SPEC-KTO-001 REQ-OPT-001, SPEC-KTO-002 REQ-OPT-001, SPEC-KTO-003 REQ-OPT-001, SPEC-KTO-004 REQ-OPT-001, SPEC-KTO-005 REQ-OPT-001, SPEC-KTO-006 REQ-OPT-001, SPEC-KTO-007 REQ-OPT-001, SPEC-KTO-008 REQ-OPT-001, SPEC-KTO-009 REQ-OPT-001, SPEC-KTO-010 REQ-OPT-001
...
export const BASE_URL_MAP = {
  ...
  KorPetTourService2: 'http://apis.data.go.kr/B551011/KorPetTourService2',
  MdclTursmService: 'http://apis.data.go.kr/B551011/MdclTursmService',
  WellnessTursmService: 'http://apis.data.go.kr/B551011/WellnessTursmService',
  PhokoAwrdService: 'http://apis.data.go.kr/B551011/PhokoAwrdService',
} as const;
