...
import { HttpStreamableTransportAdapter } from './http-streamable.adapter';
...
describe('HttpStreamableTransportAdapter', () => {
  ...
  beforeEach(() => {
    adapter = new HttpStreamableTransportAdapter();
    mockServer = {
  ...
  it('stop()이 transport와 HTTP 서버를 닫는다', async () => {
    await adapter.start(mockServer, 0);
  ...
  describe('HTTP request handling', () => {
    ...
    it('GET 요청은 transport.handleRequest로 전달된다', async () => {
      await adapter.start(mockServer, 0);
      const port = adapter.getPort();
